    ...
    >
      <div className="container mx-auto px-4 sm:px-10">
        <div className="max-w-[800px] mb-16">
          <h2
            className="text-[40px] sm:text-[48px] text-foreground font-normal leading-[1.05] mb-8 tracking-[-0.015em]"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 gap-y-10">
          {services.map((service, idx) => (
            ...
            >
              {/* Icon */}
              <div className="mb-6 shrink-0 text-[hsl(var(--green-dark))] group-hover:text-[#facc15] transition-colors duration-300">
                <service.icon className="w-8 h-8" strokeWidth={1.75} />
              </div>

              {/* Title */}
              <h3 className="text-[16.5px] font-semibold text-foreground group-hover:text-white mb-3 leading-snug transition-colors duration-300">
                {service.title}
              </h3>

              {/* Description */}
              <p className="text-[14.5px] text-muted-foreground group-hover:text-white/90 font-light leading-relaxed mt-auto transition-colors duration-300">
                {service.desc}
              </p>
            </div>
          ))}
